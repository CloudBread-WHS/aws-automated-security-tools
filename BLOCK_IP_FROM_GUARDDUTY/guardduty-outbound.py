...
def lambda_handler(event, context):
    try:
        # DynamoDB 테이블 설정
        dynamodb_table_name = '테이블 이름 입력'
        dynamodb_client = boto3.resource('dynamodb')
        table = dynamodb_client.Table(dynamodb_table_name)

        # NACL 관리
        nacl_client = boto3.client('ec2')
        network_acl_id = 'NACL ID 입력'
        ...
        # 일관성 검증
        verify_nacl_and_db_consistency(nacl_client, dynamodb_client, network_acl_id, dynamodb_table_name, egress=True, rule_range=(20, 40))

        # NACL에서 현재 규칙 수 확인
        nacl_describe = nacl_client.describe_network_acls(NetworkAclIds=[network_acl_id])
        outbound_rules = [entry for entry in nacl_describe.get('NetworkAcls', [{}])[0].get('Entries', []) if entry.get('Egress') is True]
        current_policy_count = len(outbound_rules)

        # 현재 시간의 timestamp를 얻기 (Decimal 타입으로 변환)
        current_timestamp = Decimal(str(datetime.utcnow().timestamp()))

        # 총 정책의 갯수가 20개 이상이거나 정확히 20개일 때 오래된 정책 삭제
        ...
        # 가용한 정책 번호 할당
        egress = True  # 아웃바운드 규칙으로 설정
        new_rule_number = find_available_rule_number(nacl_client, network_acl_id, egress)

        # Ensure the new rule number is within the desired range (20 ~ 40)
        if new_rule_number > 40:
            raise ValueError("더 이상 정책을 추가할 수 없습니다. 범위를 조절하세요.")
...
def verify_nacl_and_db_consistency(nacl_client, dynamodb_client, network_acl_id, dynamodb_table_name, egress=True, rule_range=None):
    if egress:
        # 예외로 처리할 정책 번호
        ...
        rules_key = 'Entries'
        nacl_rules = [
            entry for entry in nacl_describe.get('NetworkAcls', [{}])[0].get(rules_key, [])
            if entry.get('RuleNumber') not in skip_rule_numbers and (rule_range is None or rule_range[0] <= int(entry.get('RuleNumber')) <= rule_range[1])
        ]
...
response = lambda_client.invoke(
    ...
    Payload=json.dumps({"content" : "guardduty-outbound 함수 동작 "}),
